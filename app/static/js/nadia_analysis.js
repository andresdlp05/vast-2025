...
function init_nadia_analysis() {
    ...
    function initTabSwitching() {
        ...
        d3.selectAll(".analysis-tab").on("click", function(event) {
            ...
            // CRITICAL FIX: Re-trigger visualization for the active tab with delay
            if (analysisData) {
                setTimeout(() => {
                    console.log(`Re-initializing ${tabName} tab after switch`);
                    try {
                        switch(tabName) {
                            case 'timeline':
                        ...
                        console.error(`Error reinitializing ${tabName} tab:`, error);
                    }
                }, 100);
            }
        });
